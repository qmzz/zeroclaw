...
## 一期改造说明（Claw Code 理念吸收版）

本分支已完成基于 **Claw Code 优秀理念** 的一期改造收尾，重点不是照搬架构，而是把它最有价值的治理能力落到 zeroclaw 上，同时保持 zeroclaw 的轻量特性。

### 已完成的核心增强

| 能力 | 说明 | 状态 |
|------|------|------|
| **结构化诊断面** | `/doctor` 与 `/status` 能更系统地做排障和状态查看 | ✅ 已完成 |
| **确定性压缩优先** | 优先采用可解释、可复现的上下文压缩，而不是只依赖 LLM 摘要 | ✅ 已完成 |
| **策略引擎接入** | 工具调用增加权限校验与上下文策略判定 | ✅ 已完成 |
| **失败分类与恢复路径** | 故障会被分类记录，并具备恢复动作基础能力 | ✅ 已完成 |
| **分层配置加载** | 支持 workspace / project / local 三层配置覆盖 | ✅ 已完成 |
| **健康监控基础设施** | 组件级健康状态追踪与错误标记 | ✅ 已完成 |

### 当前阶段验收状态

- ✅ `cargo check` 通过
- ✅ `cargo test --lib --no-run` 通过
- ✅ `doctor::tests::*` 21/21 通过
